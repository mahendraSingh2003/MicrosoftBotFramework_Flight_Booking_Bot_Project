...
const { getFilteredFlights } = require('../services/filterFlight');
const { CardFactory } = require('botbuilder');
const { translateText } = require('../services/translatorService');

const SHOW_FILTER_DIALOG = 'SHOW_FILTER_DIALOG';

class ShowFilteredFlights extends ComponentDialog {
  constructor(conversationData, id = SHOW_FILTER_DIALOG) {
    super(id);

    this.conversationData = conversationData;

    this.addDialog(new WaterfallDialog(SHOW_FILTER_DIALOG, [
      this.showResults.bind(this)
    ]));

    this.initialDialogId = SHOW_FILTER_DIALOG;
  }

  async showResults(step) {
    const { from, to, date, adults, airline, maxPrice, layovers, userDurationInput, travelClass } = step.options;

    const convData = await this.conversationData.get(step.context, {});
    const userLang = convData.language || 'en';

    try {
      const flights = await getFilteredFlights(from, to, date, adults, airline, maxPrice, layovers, userDurationInput, travelClass);
      ...
      const foundMsg = await translateText(`✅ Found ${flights.length} Filtered flights ✈️ from **${from}** to **${to}** on **${date}**:`, userLang);
...
module.exports = { ShowFilteredFlights };
